...
export default function BazaresDirectoryClient({ bazaresData }: { bazaresData: any[] }) {
  ...
  const cities = useMemo(() => Array.from(new Set(bazaresData.map((b) => b.ciudad))), [bazaresData]);
  const types = useMemo(() => Array.from(new Set(bazaresData.map((b) => b.tipo))), [bazaresData]);
